    ...
    public class CubeTower : MonoBehaviour
    {
        ...
        public void RemoveCubeFromTower(GameObject hitFromGameObject)
        {
            ...
            if (towerCube)
            {
                ...
                towerCube.ReturnCubeToPoolAfterTime(physicsActiveTime);
        ...
        private void InitializePool()
        {
            ...
            {
                cubeGO = GameObject.Instantiate(cubeHolderPrefab, transform);
                towerCube = UtilityNS.Utilities.GetComponentFromGameObject<TowerCube>(cubeGO);
                cubes.Add(towerCube);
        ...
        private TowerCube GetCubeFromPool() =>
          cubes.FirstOrDefault(x => !x.gameObject.activeInHierarchy) ?? cubes.Where(x => !x.CubeInTower).First();
